...
public class NevadoConnectionFactory
		implements ConnectionFactory, QueueConnectionFactory, TopicConnectionFactory, Serializable, Referenceable {
	...
	private volatile AWSCredentials _awsCredentials = null;
	...
	public NevadoQueueConnection createQueueConnection() throws JMSException {
		...
		NevadoQueueConnection connection = null;
		if (_awsCredentials == null) {
			connection = new NevadoQueueConnection(
	...
	public NevadoQueueConnection createQueueConnection(String awsAccessKey, String awsSecretKey) throws JMSException {
		checkSQSConnectorFactory();
		NevadoQueueConnection connection = new NevadoQueueConnection(
				_sqsConnectorFactory.getInstance(awsAccessKey, awsSecretKey, _awsSQSEndpoint, _awsSNSEndpoint));
		initializeConnection(connection);
		return connection;
	}
	...
	public NevadoConnection createConnection() throws JMSException {
		checkSQSConnectorFactory();
		NevadoConnection connection = null;
		if (_awsCredentials == null) {
			connection = new NevadoConnection(
					_sqsConnectorFactory.getInstance(_awsAccessKey, _awsSecretKey, _awsSQSEndpoint, _awsSNSEndpoint));
		} else {
			connection = new NevadoConnection(
					_sqsConnectorFactory.getInstance(_awsCredentials, _awsSQSEndpoint, _awsSNSEndpoint));
	...
	public Reference getReference() throws NamingException {
		Reference reference = new Reference(NevadoConnectionFactory.class.getName(),
				new StringRefAddr(JNDI_AWS_ACCESS_KEY, _awsAccessKey), NevadoReferencableFactory.class.getName(), null);
		...
		if (_durableSubscriberPrefixOverride != null) {
			reference.add(new StringRefAddr(JNDI_DURABLE_SUBSCRIBER_PREFIX_OVERRIDE,
					_durableSubscriberPrefixOverride.toString()));
		}
